...
运行步骤：
1.新建一个叫Conference的数据库，执行CreateConferenceTables SQL脚本，创建该数据库中的相关表；
2.新建一个叫Conference_EQueue的数据库，执行CreateEQueueTables SQL脚本，创建该数据库中的相关表；
3.新建一个叫Conference_ENode的数据库，执行CreateENodeTables SQL脚本，创建该数据库中的相关表；
4.Hosts目录下的工程都是顶层宿主工程。我们先需要修改每个宿主工程里App.config或Web.config里配置的数据库连接字符串；需要先确保数据库连接是正确的；
