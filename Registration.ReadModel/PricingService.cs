    ...
    public class PricingService : IPricingService
    {
        ...
        public OrderTotal CalculateTotal(Guid conferenceId, IEnumerable<SeatQuantity> seats)
        {
            ...
            foreach (var item in seats)
            {
                ...
                lineItems.Add(new OrderLine { SeatType = item.SeatType, Quantity = item.Quantity, UnitPrice = seatType.Price, LineTotal = Math.Round(seatType.Price * item.Quantity, 2) });
            }
            return new OrderTotal(lineItems.ToArray(), lineItems.Sum(x => x.LineTotal));
        }
    }
}
